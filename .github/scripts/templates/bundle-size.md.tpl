...
> [!NOTE]
> **How to read this:** sizes are raw (uncompressed) bytes of the client assets in `.next/static`, so they are larger than what users download over the wire (gzip/brotli typically shrink JS and CSS substantially, see [MDN on compression](https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/Compression)). Only files in this build are counted; server code and `public/` are not included.
>
